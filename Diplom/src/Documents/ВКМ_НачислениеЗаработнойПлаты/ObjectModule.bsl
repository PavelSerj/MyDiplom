...
Процедура РасчитатьОтпуск()
	...
	//далее соединяю ВТ с Регистром расчета ОсновныеНачисления через виртуальную таблицу устанавливая период (получаю данные за 12 мес), помещаю в ВТ
	//далее две ВТ и снова тот же регистр расчета но данные уже на дату документа
	//выше данные получаю через запрос, делаю проводку, добавляю отпуск
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ
		|	ВТ_Док.Сотрудник КАК Сотрудник,
		|	СУММА(ВКМ_ОсновныеНачисленияДанныеГрафика.ВКМ_ЗначениеФактическийПериодДействия) КАК
		...
		|ВЫБРАТЬ
		|	ВТ_Док.Сотрудник КАК Сотрудник,
		|	ВТ_ПериодОклад.Результат КАК РезультатЗаПериод,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение = Движения.ВКМ_ОсновныеНачисления[ВыборкаДетальныеЗаписи.НомерСтроки - 1];	
		Движение.Результат = (ВыборкаДетальныеЗаписи.РезультатЗаПериод / ВыборкаДетальныеЗаписи.ФактическийПериодДействияЗаПериод) / ВыборкаДетальныеЗаписи.ФактическийПериодДействия;
