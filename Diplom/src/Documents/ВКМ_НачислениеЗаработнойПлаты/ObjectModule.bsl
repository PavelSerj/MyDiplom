...
Процедура СформироватьДвиженияОсновныеНачисления(СтатусОтказ)
	...
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда //значит все заполнено
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			//создаю движение по удержанию, для получения данных базы
...
Процедура РасчитатьОтпуск()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник;
		//списываю количество дней отпуска, для получения отчета в дальнейшем
...
Процедура РасчитатьУдержания()
	...
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("ВидРасчета", ПланыВидовРасчета.ВКМ_Удержания.Налог);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Движение.Результат = ВыборкаДетальныеЗаписи.РезультатБаза * 0.13;
